...
import { LanguageSwitch } from "./features/workspaceV2/LanguageSwitch";
import { WorkspaceCanvas } from "./features/workspaceV2";
import { fetchSessionBootstrap } from "./services/sessionApi";
...
function PreparingWorkspaceScreen({
  ...
  onReady: (
    data: SessionBootstrapResponse | null,
  ...
  }, [profile, selectedCharacter, outputLanguage]);

  return (
    <main className="stage-screen onboarding-screen">
      <section className="stage-card guide-card" aria-busy="true">
        <div className="stage-heading">
          <span>Almost there</span>
          <h1>Lyra is shaping your roadmap</h1>
          <p>
            We are sending your answers to the Ayna backend, then to Gemini, so
            your first canvas matches your real context.
          </p>
        </div>
        <p className="guide-message">
          <ShiningText text="Personalizing identity, interests, and roadmap visuals..." />
        </p>
      </section>
    </main>
  );
  ...
  return (
    <main className="stage-screen">
      <section className="stage-card guide-card" aria-labelledby="guide-title">
        ...
        <div className="stage-actions character-stage-actions">
          <LanguageSwitch variant="comfortable" />
          <span>
            Selected:{" "}
            {
...
function WorkspaceScreen({
  ...
  return (
    <main className="workspace-screen" aria-label="Ayna AI workspace">
      {bootstrapError ? (
        <div
          className="workspace-bootstrap-notice"
          role="status"
          style={{
        ...
        >
          Gemini personalization unavailable — showing the local preview instead.{" "}
          <span style={{ opacity: 0.85 }}>{bootstrapError}</span>
        </div>
      ) : null}
...
function App() {
  ...
  const [aiBootstrap, setAiBootstrap] =
  ...
  if (phase === "preparing-workspace") {
    return (
      <PreparingWorkspaceScreen
        onReady={(data, error) => {
          setAiBootstrap(data);
